...
def render() -> None:
    ...
    with col1:
        phi1_str = input_with_history("Φ_1(x, y)", "functions_2d", "vec_phi1", default_val="x^2 - y^2")
        save_to_history_button("functions_2d", phi1_str, "Φ_1")
    with col2:
        phi2_str = input_with_history("Φ_2(x, y)", "functions_2d", "vec_phi2", default_val="2*x*y")
        save_to_history_button("functions_2d", phi2_str, "Φ_2")
        
    st.subheader("Zbiory")
    col3, col4 = st.columns(2)
    with col3:
        c_str = input_with_history("Zbiór C ⊆ R² (do obrazu)", "sets_r2", "vec_c", default_val="[-1, 1]x[-1, 1]")
        save_to_history_button("sets_r2", c_str, "Zbiór C")
    with col4:
        b_str = input_with_history("Zbiór B ⊆ R² (do przeciwobrazu)", "sets_r2", "vec_b", default_val="x^2 + y^2 <= 1")
    ...
    if st.button("Oblicz i Rysuj", type="primary"):
        ...
        if c_set:
            u_list, v_list = compute_image_points(phi1_res.expr, phi2_res.expr, c_set, source_bounds, resolution=100)
            with plot_col1:
                st.write("### Obraz Φ(C)")
                fig_img = go.Figure(data=go.Scatter(x=u_list, y=v_list, mode='markers', marker=dict(color='blue', size=3)))
